...
$LDAP_INTERNAL_PHONE_FIELD="telephonenumber";	// Атрибут LDAP, в котором должен храниться внутренний номер телефона. По умолчанию используется значение {othertelephone}.
$LDAP_CITY_PHONE_FIELD="";		// Атрибут LDAP, в котором должен храниться городской номер телефона. По умолчанию используется значение {telephonenumber}.
...
//----------------------------------------------------------------------------
$SHOW_COMPUTER_FIELD=true; // Столбец компьютер по умалчяанию false
$SHOW_DEPUTY=true;
...
// Кабинеты
//----------------------------------------------------------------------------
$HIDE_ROOM_NUMBER=false; //true - кабинеты НЕ показывать , folse кабинеты показывать
//----------------------------------------------------------------------------
...
$HIDE_HOME_PHONE_FIELD=false;		// Скрыть домашний номер телефона (true) или нет (false)?
...
// Экспорт в PDF
//----------------------------------------------------------------------------
$ENABLE_PDF_EXPORT=true; // Меню экспрта в PDF вкл/откл

$PDF_TITLE="Справочник сотрудников компании"; //Заголовок справочника при экспорте в PDF.
$PDF_SECOND_LINE="тел.: +7 (495) xxx-xx-xx"; //Постоянные контакты, которые будут выводится в PDF (Например, номер факса)
...
// Экспорт в EXEL
//----------------------------------------------------------------------------
$ENABLE_EXEL_EXPORT=true; // Меню экспрта в PDF вкл/откл
...
//----------------------------------------------------------------------------
$PHOTO_DIR="./temp/".$COMPANY_DIR."/img"; //Директория для хранения фотографий
...
$PHOTO_MAX_WIDTH=""; //Максимальная ширина фото в пикселях
...
$XMPP_ENABLE=false; // Закладка с права, отправить сообщение
