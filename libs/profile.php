	...
	if($WhoAreYou!==false)
		...
		if($Login=$ldap->getValue($_COOKIE['dn'], "userprincipalname"))
			...
			if(in_array($Login, $ADMIN_LOGINS))
				echo"<li><a href=\"".$_SERVER['PHP_SELF']."?menu_marker=si_staffedit\">".$localization->get("admin_panel")."</a></li>";
			}
		...
		echo"<li><a href=\"newwin.php?menu_marker=si_employeeview&dn=".$_COOKIE['dn']."\" data-lightview-type=\"iframe\" data-lightview-options=\"width: '80%', height: '100%', keyboard: {esc: true}, skin: 'light'\" class=\"lightview\">".$localization->get("profile")."</a></li>";
		
		if($VACATION)
			{
			echo"<li><span id=\"Vac\">".$localization->get("vacation_plan")."<big><big>&rarr;</big></big> <span class=\"\">
		...
		if(@!$_SERVER['REMOTE_USER'])
			echo"<li><a href=\"".$_SERVER['PHP_SELF']."?iamnot=1\" title=\"Нет! \">".$localization->get("exit")."</a></li>";
		echo"</ul></fieldset>";	
		}
	}
?>