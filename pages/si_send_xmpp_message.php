...
if($XMPP_LDAP_GROUPS_ENABLE)
    ...
    $Groups = $ldap->getArray($ou,  $Filter, array($LDAP_XMMP_GROUP_TITLE_FIELD, $CONFIG_LDAP_ATTRIBUTE['LDAP_DISTINGUISHEDNAME_FIELD']), array($LDAP_XMMP_GROUP_TITLE_FIELD), "ASC", true);

    $i=0;
    foreach($Groups[$CONFIG_LDAP_ATTRIBUTE['LDAP_DISTINGUISHEDNAME_FIELD']] AS $key=>$value)
        {
        if($i==0)
            echo"<div class=\"xmpp_groups\">";
        echo "<div><input id=\"group_".$i."\" type=\"checkbox\" name=\"groups[".$i."]\" value=\"".$Groups[$CONFIG_LDAP_ATTRIBUTE['LDAP_DISTINGUISHEDNAME_FIELD']][$key]."\"/><label for=\"group_".$i."\">".$Groups[$LDAP_XMMP_GROUP_TITLE_FIELD][$key]."</label></div>";
        $i++;
        }
    if($i>0)
        echo"</div>";
...
?>
    ...
    <input type="submit" name="" value="<?php echo $localization->get('send_message'); ?>" />
</div>
</form>