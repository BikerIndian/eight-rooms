...
class Phones
{
    ...
    function __construct($CONFIG_PHONE, $CONFIG)
    {
    $this->PHONE_CODES = $CONFIG_PHONE['PHONE_CODES'];
    $this->PROVIDER_DESC = $CONFIG_PHONE['PROVIDER_DESC'];
    ...
    function get_phone_attr($phone = '', $convert = true, $trim = true)
    {
        ...
        if (strlen($phone)>7)
        foreach ($this->PHONE_CODES as $countryCode=>$data)
        {
            ...
            if (substr($phone, 0, $codeLen)==$countryCode)
            {
                ...
                $PhoneAttr['format_phone']=($plus ? "+" : "").$countryCode.'('.$cityCode.')'.$this->phoneBlocks($phone, $this->CONFIG_PHONE['FORMAT_PHONE_BLOCKLEN']);
                $PhoneAttr['clear_phone']=($plus ? "+" : "").$countryCode.$cityCode.$phone;
                $PhoneAttr['original_phone']=$OriginalPhone;
                if ( array_key_exists($countryCode,  $this->PROVIDER_DESC) AND array_key_exists($cityCode,  $this->PROVIDER_DESC[$countryCode]) )
                    $PhoneAttr['provider_desc']= $this->PROVIDER_DESC[$countryCode][$cityCode];
                else
        ...
        $PhoneAttr['format_phone']=($plus ? "+" : "").$this->phoneBlocks($phone, $this->CONFIG_PHONE['FORMAT_PHONE_BLOCKLEN']);
